...
class TrendingNewUsers extends StatefulWidget {
  const TrendingNewUsers({Key? key}) : super(key: key);

  @override
  State<TrendingNewUsers> createState() => _TrendingNewUsersState();
}

class _TrendingNewUsersState extends State<TrendingNewUsers> {
  int page = 1;
  ...
  Widget build(BuildContext context) {
        ...
        Row(
          ...
          children: [
            ...
            TextButton(
              ...
              onPressed: () {
                searchController!.newusers.value=!searchController!.newusers.value;
                page++;
                getTrending(context,limit: 10);
              },
              child: Obx(()=>Text(searchController!.newusers.value?'view less':'view all')),
            ),
          ],
        ),
    Obx(() => searchController!.newusers.value?GridView.builder(
    ...
    itemCount: searchController!.newUserList.length,
    itemBuilder: (context, index) {
    AcceptUserDetail user =
    searchController!.newUserList.elementAt(index);
          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Text(
                    "${user.state != null ?user.state!.length<10?
                    user.state ?? "" :
                    '${user.state!.substring(0,9)}' : ""} ${user.country ?? ""}", style: GoogleFonts.nunitoSans(
                    fontWeight: FontWeight.w400,
                    fontSize: ScreenUtil().setSp(10),
                    color: black)),
              ),
              InkWell(
                onTap: () {
                  if (user.amIFollowing ==
                    ...
                    int sunIndex = index;
                    searchController!.newUserList.removeAt(index);
                    searchController!.newUserList
  ...
  getTrending(BuildContext context,{int limit=5}){
    Api.get.call(context,
        method: "user/trending/user",
        param: {
          "page":page.toString(),
        ...
        isLoading: false, onResponseSuccess: (Map object) {
          ...
          if(result.userdata!.isNotEmpty) {
            searchController!.newUserList.addAll(result.userdata!);
          }else{
