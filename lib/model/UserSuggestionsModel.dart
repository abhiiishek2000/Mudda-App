...
class UserSuggestionsModel {
  int? status;
  String? message;
  List<AcceptUserDetail>? data;
  String? path;
  int? inviteCount;


  UserSuggestionsModel({this.status, this.message, this.data,this.inviteCount,this.path});

  UserSuggestionsModel.fromJson(Map<dynamic, dynamic> json) {
    status = json['status'];

    message = json['message'];
    path = json['path'];
    if (json['data'] != null) {
      data = <AcceptUserDetail>[];
      json['data'].forEach((v) {
        data!.add(AcceptUserDetail.fromJson(v));
      });
    }
